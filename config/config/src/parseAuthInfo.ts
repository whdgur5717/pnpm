...
function parseTokenHelper (source: string): TokenHelper {
  ...
  for (const char of source) {
    ...
    if (!/[a-z0-9_=+-]/i.test(char)) { // eslint-disable-line regexp/prefer-w
  ...
  if (command.length === 0) {
    throw new TokenHelperEmptyCommandError()
  }
...
export class TokenHelperEmptyCommandError extends PnpmError {
  constructor () {
    super('TOKEN_HELPER_EMPTY_COMMAND', 'Command cannot be empty')
  }
}
